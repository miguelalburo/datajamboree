...
params <- list(
  ...
  # less strict for gene co-abundance networks
...
data      <- dims$data.matrix   # samples x genes
...
# saveRDS(sft, file.path(params$out_dir, "sft.rds"))

# Pre ran
sft <- readRDS(file.path(params$out_dir, "sft.rds"))
...
pdf(file.path(params$out_dir, "gene_dendrogram.pdf"), width = 14, height = 9)
plotDendroAndColors(
  gene_tree,
...
cat("\nEdges per module:\n")
edge_list %>% group_by(module) %>%
  count(module, name = "n_edges") %>% print()

cat("\nHub genes per module:\n")
hub_table %>% group_by(module) %>%
  count(module, name = "n_hubs") %>% print()
