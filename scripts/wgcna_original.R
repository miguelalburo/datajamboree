...
# - Exports a combined edge list and hub metabolite table for significant modules
#
...
params <- list(
  ...
  # Hub metabolite: top % by degree within each module
  hub_quantile     = 0.95,
...
# TOM <- readRDS(file.path(params$out_dir, "TOM.rds"))
dissTOM <- 1 - TOM
...
pdf(file.path(params$out_dir, "02_metabolite_dendrogram.pdf"), width = 14, height = 9)
plotDendroAndColors(
  metabolite_tree,
...
for (name in names(conds)) {
  ...
  results[[name]] <- full_results
  
  write.csv(full_results,
            file.path(params$out_dir, paste0("module_trait_",name,".csv")),
            row.names = FALSE)
...
for (result in results) {
  ...
  sig_modules <- rbind(sig_modules, significant)
}
...
# Build Combined Edge List & Hub Metabolite Table -------------------------
...
# write.csv(hub_table,
#           file.path(params$out_dir, "hub_metabolites.csv"),
#           row.names = FALSE)
...
# cat(sprintf("hub_metabolites.csv    : %d hub metabolites across %d modules\n",
